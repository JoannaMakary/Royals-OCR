...
def process_all_images():
    ...
    img = cv2.imread("./dataset/frame1.tiff")
    detect_mouse(img)
...
def crop_hover_list(img, cursor_coord_x, cursor_coord_y):
    ...
    for contour in contours:
        ...
        if cv2.contourArea(contour) < 4000 or cv2.contourArea(contour) > 10000:
              continue
...
def crop_hover_popup(img, cursor_coord_x, cursor_coord_y):
    ...
    # canny = cv2.Canny(grayimage, 50, 500)
    ...
    for contour in contours:
        # # Contour area determines what it will contour/crop
        # if cv2.contourArea(contour) < 11000 or cv2.contourArea(contour) > 90000:
        #     continue
...
def process_hover_popup(item_list_name, item_list_price):
    ...
    # cv2.imshow("erosionname", erosion)
    cv2.waitKey(0)
